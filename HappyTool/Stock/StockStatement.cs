...
namespace HappyTool
{
            ...
            for (int i = 0; i < count; i++) {
                try {
                    ...
                    if (buyCount == 0) {
                        ...
                        stockBot.addStockData(StockData);
                        stockBot.requestStockData(PublicVar.initType, code);
                    }
                    else {
                        ...
                        stockBot.addStockData(buyedStockData);
                        stockBot.requestStockData(PublicVar.initType, code);
                    }
    ...
    class AccountMoneyStatement : OrderStockStatement
    {
        ...
        public override void receive(object sender, AxKHOpenAPILib._DKHOpenAPIEvents_OnReceiveTrDataEvent apiEvent)
        {
            try {
                ...
                Program.happyTool_.stockDlg_.updateAccountMoney(money);
    ...
    class AccountMoney2Statement : OrderStockStatement
    {
        ...
        {
            try {
                string temp1 = this.getData("출금가능금액(예수금)", apiEvent, 0).Trim();
                string temp2 = this.getData("주문가능금액(예수금)", apiEvent, 0).Trim();
                int money = 0;
                if (temp1.Length != 0) {
                    money = int.Parse(temp1);
                }
             ...
             //   Program.happyTool_.updateAccountMoney(money);
        ...
        public override void receive(object sender, AxKHOpenAPILib._DKHOpenAPIEvents_OnReceiveTrDataEvent apiEvent)
        {
            ...
            for (int i = 0; i < count; i++) {
                try {
                    ...
                    StockData stockData = new StockData(code, name);
                    StockBot.getInstance.addStockData(stockData);
                }
                catch (AccessViolationException execption) {
                    Logger.getInstance.print(Log.에러, "[receive:{0}] {1}\n{2}\n{3}", this.ToString(), execption.Message, execption.StackTrace, execption.InnerException);
                }
                //Logger.getInstance.print(Log.조회, "종목코드:{0} | 종목명:{1} | 현재가:{2} | 전일대비:{3} | 등락률:{4}",
                //    this.getData("종목코드", apiEvent, i),
                //    this.getData("종목명", apiEvent, i),
                //    this.getData("현재가", apiEvent, i),
                //    this.getData("전일대비", apiEvent, i),
                //    this.getData("등락률", apiEvent, i));
            }
        }
    }
    ...
    class StockInfoStatement : OrderStockStatement
    {
        ...
        {
            try {
                StockData stockData = StockBot.getInstance.stockData(stockCode_);
                if (stockData == null) {
                    return;
                }
                ...
                lock (stockData.dataLock_) {
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        CandleData priceData = new CandleData(dateTime, price, startPrice, highPrice, lowPrice);
    ...
    //--------------------------------------------------------//
    // 주식 종목의 차트 조회
    class HistoryThe15MinStock : StockInfoStatement
    {
        public HistoryThe15MinStock(int stockCode)
        {
            stockCode_ = stockCode;
            requestName_ = "주식15분차트조회요청";
            requestCode_ = "OPT10080";
            priceType_ = PRICE_TYPE.MIN15;
        }
    ...
    class HistoryThe1HourStock : StockInfoStatement
    {
        public HistoryThe1HourStock(int stockCode)
        {
            stockCode_ = stockCode;
            requestName_ = "주식1시간봉차트조회요청";
            requestCode_ = "OPT10080";
            priceType_ = PRICE_TYPE.HOUR1;
        }
    ...
    class HistoryThe4HourStock : StockInfoStatement
    {
        public HistoryThe4HourStock(int stockCode)
        {
            stockCode_ = stockCode;
            requestName_ = "주식4시간봉차트조회요청";
            requestCode_ = "OPT10080";
            priceType_ = PRICE_TYPE.HOUR4;
        }
        ...
        protected override bool setInput()
        {
            ...
            // 틱범위 = 1:1분, 3:3분, 5:5분, 10:10분, 15:15분, 30:30분, 45:45분, 60:60분
            if (this.setParam("틱범위", "240") == false)
            {
                return false;
            }
            //수정주가구분 = 0 or 1, 수신데이터 1:유상증자, 2:무상증자, 4:배당락, 8:액면분할, 16:액면병합, 32:기업합병, 64:감자, 256:권리락
    ...
    class HistoryTheWeeksStock : StockInfoStatement
    {
        public HistoryTheWeeksStock(int stockCode)
        {
            stockCode_ = stockCode;
    ...
    class BuyStock : TradingStatement
    {
        ...
        protected override bool setInput()
        {
            ...
            if (price == 0) {
                ...
                price = stockData.nowPrice(PRICE_TYPE.HOUR1);
            }
            return true;
        }
    ...
    class SellStock : TradingStatement
    {
        public SellStock(BuyedStockData ownerStock)
        {
            ...
            tradingPrice_ = ownerStock.nowPrice(PRICE_TYPE.HOUR1);
        }
